...
public class CSARUtil {
	...
	/**
	 * Creates a CSAR file based on a YAML {@link org.opentosca.yamlconverter.yamlmodel.yaml.element.ServiceTemplate},
	 * a String containing the generated XML, a String containing the generated and corresponding XSD and a filename for
	 * that CSAR file. A CSAR file is simply a ZIP folder. The method creates and prepares three temporal folders:
	 * {@link #META_FOLDER}, {@link #DEFINITIONS_FOLDER} and {@link #TOSCA_TYPES_FOLDER} which contain different files.
	 * {@link #META_FOLDER} contains the CSAR meta file called {@link #META_FILENAME}. {@link #DEFINITIONS_FOLDER} contains
	 * the generated XML and two XML documents to provide the Base- and SpecificTypes for TOSCA XML. {@link #TOSCA_TYPES_FOLDER}
	 * contains the XSD files for the mentioned XML documents (exception: the XSD for the generated XML file can be
	 * found in {@link #DEFINITIONS_FOLDER}).
	 * <br />
	 * The CSAR file will be put to the current working directory, e.g. if you're using the program as a JAR, then the
	 * CSAR is put into the same folder where the JAR is.
	 * <br />
	...
	private static void prepareFoldersWithFiles(final ServiceTemplate serviceTemplate, final String xml, final String xsd, final FileUtil fileUtil) throws IOException {
		// prepare folders
